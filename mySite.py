...
def textmining():
	...
	if request.method == 'POST':
		...
		if(result.split(':')[0] == 'Stress Detected'):
			text_dep = 1

		return render_template('textmining.html',name=username,symptoms=symptoms,result=result)			    
...
def voice():
	...
	if request.method == 'POST':
		if request.form['sub'] == 'Speak':
			...
			if(result.split(':')[0] == 'Stress Detected'):
				speech_dep = 1
			return render_template('voice.html',name=username,symptoms=symptoms,result=result)
...
def final_result():
	...
	stress = "Video  = "+str(vid_dep) + "," + "Text Mining  = "+str(text_dep)+","+"Speech  = "+str(speech_dep)
	score = ((vid_dep+speech_dep+text_dep)/3)*100
	return render_template('result.html',stress=stress,score=score)
